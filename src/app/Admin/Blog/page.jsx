...
export default function BlogPage() {
    ...
    const fetchData = async (filter) => {
        ...
        if (res.success) {
            setData((res.Items || []).map((item, idx) => ({ ...item, key: item.BLOG_ID, STT: idx + 1 })));
        }
    };
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text) => {
                if (!text) return '—';
                try { return FormatDateTime(new Date(text)); } catch { return text; }
            },
        },
        {
            title: 'Chức năng',
    ...
    const statsData = useMemo(() => {
        ...
        data.forEach(blog => {
            ...
            if (!map[key]) map[key] = { key, AUTHOR_NAME: blog.AUTHOR_NAME || key, POST_COUNT: 0, DATES: [] };
            map[key].POST_COUNT += 1;
            if (blog.CREATED_DATE) map[key].DATES.push(blog.CREATED_DATE);
        });
        return Object.values(map).map((s, idx) => ({
            ...s, STT: idx + 1,
            LATEST_DATE: s.DATES.length ? s.DATES.sort((a, b) => new Date(b) - new Date(a))[0] : null,
        }));
    }, [data]);
    ...
    const statsColumns = [
        ...
        { title: 'Số bài viết', dataIndex: 'POST_COUNT', width: 130, sorter: (a, b) => a.POST_COUNT - b.POST_COUNT, render: c => <Tag color="blue">{c} bài</Tag> },
        { title: 'Bài mới nhất', dataIndex: 'LATEST_DATE', width: 200, render: t => t ? (() => { try { return FormatDateTime(new Date(t)); } catch { return t; } })() : '—' },
    ];

    const tabItems = [
        {
            key: 'list',
            label: <><UnorderedListOutlined /> Danh sách bài viết</>,
            children: (
                <>
                    <div className="d-flex gap-2 mb-3 pt-3 flex-wrap align-items-center">
                        ...
                        <Tag color={isAdmin ? 'red' : 'blue'} style={{ height: 40, lineHeight: '38px', fontSize: 14, padding: '0 16px', borderRadius: 20 }}>
                            {isAdmin ? 'ADMIN' : 'Nhân viên'}
                        </Tag>
                    </div>
                    <div className={`${styles.transparentTable}`}>
                        <Table
                            rowSelection={isAdmin ? { onChange: (_, rows) => setSelectedRows(rows), selectedRowKeys: selectedRows.map(r => r.key) } : undefined}
                            bordered
                            columns={columns}
                            dataSource={data}
        ...
        {
            key: 'stats',
            label: <><BarChartOutlined /> Thống kê người đăng</>,
            children: (
                <div className="pt-3">
                    <div className="mb-3">
                        <strong>Tổng số bài viết:</strong> <Tag color="blue">{data.length} bài</Tag>
                        <strong className="ms-3">Số người đăng:</strong> <Tag color="purple">{statsData.length} người</Tag>
                    </div>
                    <Table bordered columns={statsColumns} dataSource={statsData} className={styles.gridTable} pagination={false} />
                </div>
            ),
        },
    ];
    ...
    return (
        <section>
            ...
            <Modal
                title={form.BLOG_ID ? 'Chỉnh sửa bài viết' : 'Thêm bài viết mới'}
                open={showModal}
                onOk={handleSave}
