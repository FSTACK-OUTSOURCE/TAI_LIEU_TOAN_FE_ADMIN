...
import { Radio, Button, Table, Input, Modal, Select } from 'antd';
import { PlusOutlined, EyeInvisibleOutlined, EyeTwoTone, SafetyCertificateOutlined } from '@ant-design/icons';
...
const Account: React.FC<any> = () => {
    ...
    const [roleModal, setRoleModal] = useState<{ open: boolean; record: any }>({ open: false, record: null });
    ...
    const columns = [
        ...
        { title: 'Điện thoại', dataIndex: 'PHONE_NUMBER', width: 150, key: 'PHONE_NUMBER' },
        {
            title: 'Số dư',
            dataIndex: 'BALANCE',
            width: 150,
        ...
        },
        {
            ...
            render: (_: any, record: any) => {
                ...
                const color = role === 'admin' ? '#f5222d' : role === 'editer' ? '#1677ff' : '#aaa';
                const label = role === 'admin' ? 'Admin' : role === 'editer' ? 'Editer' : 'Không';
        ...
        },
        {
            ...
            render: (_: any, record: any) => (
                <Radio.Group>
                    <Radio.Button onClick={() => { setShowPopup(true); setUserId(record.USER_ID); }}>Nạp</Radio.Button>
                    <Radio.Button onClick={() => { setShowPopupDocument(true); setUserId(record.USER_ID); }}>Tài liệu</Radio.Button>
                    <Radio.Button onClick={() => openRoleModal(record)} style={{ color: '#fa8c16' }}>
    ...
    const togglePopup = async () => { await getDataUser(); setShowPopup(false); };
    const togglePopupDocument = async () => { await getDataUser(); setShowPopupDocument(false); };

    const onChange = (val: any) => setUserData(val);
    const onSave = async () => {
        const response = await postUserInfo(userData);
        if (response.success) { setShowPopupCreate(false); getDataUser(); }
    };
    ...
    return (
        <section>
                                        ...
                                        <form className="mx-1 mx-md-4">
                                            <div className="d-flex flex-row align-items-center mb-4">
                                                <div className="form-outline flex-fill mb-0">
                                                    <label>Email</label>
                                                    <input type="text" className="form-control" placeholder='Nhập email' value={userData?.EMAIL || ''} onChange={(e) => onChange({ ...userData, EMAIL: e.target.value })} />
                                                </div>
                                            </div>
                                            <div className="d-flex flex-row align-items-center mb-4">
                                                <div className="form-outline flex-fill mb-0">
                                                    <label>Điện thoại</label>
                                                    <input type="text" className="form-control" placeholder='Nhập số điện thoại' value={userData?.PHONE_NUMBER || ''} onChange={(e) => onChange({ ...userData, PHONE_NUMBER: e.target.value })} />
                                                </div>
                                            </div>
                                            <div className="d-flex flex-row align-items-center mb-4">
                                                <div className="form-outline flex-fill mb-0">
            ...
            <div className={`col-md-12 pt-5 ${styles.contentRight}`}>
                <Filter columns={columns} onFilter={async (filter: any) => { await getDataUser(filter); }} />
                <Button className={`${styles.buttonFeature}`} onClick={() => setShowPopupCreate(true)} type="primary" shape="round" icon={<PlusOutlined />} size={size}>
                    Thêm mới
                </Button>
            </div>
